...
from app.services.cache_service import list_recent_unique_album_logs
...
async def handle(command: LogCommand, user_openid: str, msg_id: str) -> None:
    del command
    try:
        rows = await asyncio.to_thread(list_recent_unique_album_logs, LOG_FETCH_LIMIT)
    ...
    for start in range(0, total, LOG_BATCH_SIZE):
        ...
        header = f"最近访问作品（去重，{start + 1}-{start + len(batch)} / {total}）："
        lines = [header]
        for row in batch:
            created_at_text = datetime.fromtimestamp(float(row["created_at"])).strftime("%Y-%m-%d %H:%M:%S")
            lines.append(f"{created_at_text} | [{row['album_id']}] {row['album_title']}")
